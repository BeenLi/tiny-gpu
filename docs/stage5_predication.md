...
- **ARM**：经典 A32 模式**每条指令条件执行**——最高 4 位是条件域（`EQ/NE/GT/LT/…`），故 `ADDEQ`=相等才加、`MOVNE`=不等才传；条件查 CPSR 标志（由 `CMP` 或带 `S` 后缀的算术置位）。教科书例子 GCD（整段 if/else 零跳转）：
  ...
  **两条隐性账单的微架构细节**
  - **重命名税**：`ADDEQ rd,rn,rm` 语义是 `rd = cond ? rn+rm : rd_旧`，谓词假需保留旧值 → 重命名时必须把**旧 rd 的物理寄存器当第三个源**读 → 谓词指令=3 寄存器源+flags（普通 ADD 仅 2 源）。代价：①寄存器堆读口/发射队列唤醒逻辑加宽（读口面积~端口数平方）；②条件写仍照常烧一个新物理寄存器与重命名槽，压缩乱序窗口；③"读旧 rd"把**上一个写 rd 者**变成依赖，注入了重命名本要消除的假依赖，并让 flags 生产者成为众多消费者的公共前驱、拉长依赖链；④flags 部分更新引出 partial-flag 合并停顿。`CSEL rd,rn,rm,cond` 两候选值皆显式源、**写无条件**，不读旧 rd、无此税——这正是 A64 选它的精确理由。
  - **宽发射浪费**：谓词为假的指令仍占满 fetch→decode→rename→issue→execute→ROB，只省写回；if-conversion 两路内联 → 用 2k 条指令换 k 条有用功。真正代价是**机会成本**——宽机器每周期要填 N 个发射槽，死指令抢占的是本可就绪的有用指令的槽（投机则用预测正确路径的 ~99% 有用指令填满）；还额外加大 I-cache/取指足迹（常是瓶颈）、稀缺执行端口争用、以及功耗受限下的白烧能量。机器越宽、前端越瓶颈，浪费相对越重（"有富余宽度=谓词白送"是错觉）。
  - **GPU 两税皆免**：顺序执行、无激进重命名、`EXEC` 是每 wavefront 统一掩码（非每指令 3 源条件写）→ 无重命名税；SIMT 发散本就串行执行两路（无"只跑一条"的投机可选）、空闲 lane 由多 warp 隐藏 → 谓词/掩码非额外浪费。
- **x86**：有限谓词化 `CMOV`（条件传送）。
